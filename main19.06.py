...
# === Улучшенный парсер числовых значений ===
def parse_number(val):
    if pd.isna(val):
        return None
    val = str(val).replace(" ", "").replace("’", "")  # убираем пробелы/апострофы
    ...
    if val.count(",") == 1 and val.count(".") == 0:
        ...
        val = val.replace(".", "")
    # В остальных случаях оставляем только цифры и одну точку
...
def read_data():
    ...
    for col in df.columns:
        if col != "Дата":
            df[col] = df[col].apply(parse_number)  # <--- используем наш новый парсер
